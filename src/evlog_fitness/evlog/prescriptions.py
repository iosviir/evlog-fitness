# -*- coding: utf-8 -*-
"""
Created on Tue Apr 28 09:28:33 2026


Event-log construction from prescription data.

This module provides the high-level function
`evlog_from_prescriptions`, which converts longitudinal
prescription records into a process-mining–ready event log.

The pipeline:
    1. Build a daily treatment status matrix
    2. Apply measurement-related corrections
...
def evlog_from_prescriptions(
    ...
    """
    Build an event log from prescription data.

    Parameters
    ----------
    treat : pd.DataFrame
        Prescription data with at least the following columns:
            - patient_id
            - Event
    ...
    df = pd.concat([
        treat.rename(columns={
            "prescription_date_ini": "date0",
            "prescription_date_end": "date1",
        }),
        param,
    ])
    # Fill missing end dates with study end
    df["date1"] = df["date1"].fillna(
    ...
    for nid, pid in enumerate(tqdm(df["patient_id"].unique())):
        ...
        # -------------------------------
        # Apply measurement corrections
        # -------------------------------             
        status_matrix = apply_measurement_corrections(
            status_matrix,
            measure_rows,
            non_measure_rows,
